...
private let FTImageViewerAnimationDuriation : TimeInterval =  1
...
extension FTImageViewer {
    ...
    public static func showImages(_ images : [UIImage] , atIndex : NSInteger , fromSenderArray: [UIView]) {
        self.sharedInstance.showImages(images, atIndex: atIndex, fromSenderArray: fromSenderArray);
    }
}
...
public class FTImageViewer: NSObject, UIScrollViewDelegate, UIGestureRecognizerDelegate {
    ...
    public func showImages(_ images : [UIImage] , atIndex : NSInteger , fromSenderArray: [UIView]) {
        
        var resources : [FTImageResource] = []
        for image in images {
            let resource : FTImageResource = FTImageResource(image: image, imageURLString:nil)
            resources.append(resource)
        }
        self.showImages(resources, atIndex: atIndex, fromSenderArray: fromSenderArray)
    }
...
public class FTImageGridView: UIView {
    ...
    public convenience init(frame : CGRect, imageArray : [FTImageResource] , tapBlock : @escaping ((_ buttonsArray: [UIButton] , _ buttonIndex : NSInteger) ->())){
        self.init(frame: frame)
    ...
    public func showWithImageArray(_ imageArray : [FTImageResource] , tapBlock : @escaping ((_ buttonsArray: [UIButton] , _ buttonIndex : NSInteger) ->())) {
        ...
        if imageArray.count > 0 {
            ...
            for i in 0 ... imageArray.count-1 {
                ...
                if let img: UIImage = (imageArray[i]).image {
                    imageButton.setImage(img, for: .normal)
                }else if let imageURL : String = (imageArray[i]).imageURLString {
                    imageButton.kf.setImage(with: URL(string: imageURL)!, for: UIControlState.normal)
                }
