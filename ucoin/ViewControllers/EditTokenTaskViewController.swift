...
class EditTokenTaskViewController: FormViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "编辑\(tokenTask.token!.symbol ?? "")代币任务"
...
extension EditTokenTaskViewController {
    @objc private func updateTokenTask() {
        ...
        if self.pickedImages.count > 0 {
                ...
                success: {[weak self] upTokens in
                    ...
                    for upToken in upTokens {
                        weakSelf.uploadImage(upToken, success: { [weak weakSelf] () -> Void in
                            ...
                            weakSelfSub.doUpdateTokenTask(newTokenTask)
                        })
                    }
                },
            ...
            self.doUpdateTokenTask(newTokenTask)
        }
    }
    ...
    private func doUpdateTokenTask(_ tokenTask: APITokenTask) {
